...
export function ResultQuestionList({
  ...
  const [filter, setFilter] = useState<
    "ALL" | "ATTEMPTED" | "CORRECT" | "INCORRECT" | "SKIPPED"
  >("ALL");
  ...
  const correctQuestions = data.questions.filter(
  ...
  const filteredQuestions = data.questions.filter((q) => {
    ...
    if (filter === "CORRECT")
      return hasResponse && q.studentResponse?.isCorrect;
    if (filter === "INCORRECT")
      return hasResponse && !q.studentResponse?.isCorrect;
...
function QuestionCardItem({
  ...
  const contentObj =
    typeof q.question.content === "object" && q.question.content !== null
      ? q.question.content
      : {
          en: String(q.question.content || ""),
          hi: String(q.question.content || ""),
        };

  const solutionObj =
    typeof q.question.solution === "object" && q.question.solution !== null
      ? q.question.solution
      : {
          en: String(q.question.solution || ""),
          hi: String(q.question.solution || ""),
        };

  const contentStr = contentObj[language] || contentObj.en || "";
  const solutionStr = solutionObj[language] || solutionObj.en || "";
  const rawOptions = Array.isArray(q.question.options)
    ? q.question.options
    : [];
  ...
  const studentAnswers = response?.userAnswer
    ? String(response.userAnswer)
        .split(",")
        .map((s) => s.trim().toUpperCase())
    : [];
  const correctValues = q.question.correctValue
    ? String(q.question.correctValue)
        .split(",")
        .map((s) => s.trim().toUpperCase())
    : [];
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-2.5 bg-muted/40 border-b border-border/50 rounded-t-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          {isIncorrect && (
            <Badge
              ...
              variant="destructive"
            >
              <XIcon className="h-3 w-3" />-{q.negativeMarks} Incorrect
            </Badge>
          )}
          ...
          <span
            ...
            title={`Time spent on this question: ${formatSeconds(response?.timeTaken || 0)}`}
          >
            <ClockIcon className="h-3 w-3 shrink-0 opacity-80" />
            {formatSeconds(response?.timeTaken || 0)}
          </span>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {rawOptions.map((opt: any, optIdx: number) => {
                ...
                if (isOptSelected && isOptCorrect) {
                  ...
                  cardStyle =
                    "bg-emerald-500/10 border-emerald-500/60 text-foreground ring-1 ring-emerald-500/30 font-medium";
                  badgeContent = (
                  ...
                  cardStyle =
                    "bg-rose-500/10 border-rose-500/60 text-foreground ring-1 ring-rose-500/30 font-medium";
                  badgeContent = (
                    <Badge
                      className="text-[9px] uppercase px-1.5 py-0 font-bold shrink-0"
                      variant="destructive"
                    >
                  ...
                  cardStyle =
                    "bg-emerald-500/5 border-emerald-500/40 text-foreground font-medium border-dashed";
                  badgeContent = (
                    <Badge
                      className="text-[9px] uppercase px-1.5 py-0 text-emerald-600 dark:text-emerald-400 border-emerald-500/40 font-bold shrink-0"
                      variant="outline"
                    >
                      ○ Correct Answer
                    </Badge>
